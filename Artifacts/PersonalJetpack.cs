...
internal sealed class PersonalJetpack : Artifact, IAngderArtifact
{
    ...
    public override void OnTurnStart(State s, Combat c)
    {
        if (!c.isPlayerTurn)
            return;
        if (s.ship.Get(ModEntry.Instance.Rampage.Status) < 3 && s.ship.Get(ModEntry.Instance.Disrupt.Status) < 2 && s.ship.Get(ModEntry.Instance.Angdermissing.Status) > 0 && s.ship.Get(ModEntry.Instance.Fury.Status) == 0 && s.ship.Get(ModEntry.Instance.Theft.Status) <= 1)
        {
            c.Queue([
                        new AStatus()
                    {
